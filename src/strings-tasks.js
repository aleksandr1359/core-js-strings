...
function removeFirstOccurrences(str, value) {
  const firstOcc = str.indexOf(value);
  if (firstOcc >= 0) {
    const l = str.length;
    const firstPart = str.substring(0, firstOcc);
    const secondPart = str.substring(value.length + firstOcc, l);
...
function removeLastOccurrences(str, value) {
  const lastOcc = str.lastIndexOf(value);
  if (lastOcc >= 0) {
    const l = str.length;
    const firstPart = str.substring(0, lastOcc);
    const secondPart = str.substring(value.length + lastOcc, l);
    return firstPart.concat(secondPart);
  }
  return str;
}
...
function formatTime(minutes, seconds) {
  const str = '';
  return str.concat(
    minutes > 9 ? minutes : minutes.toString().padStart(2, '0'),
    ':',
...
function countVowels(str) {
  ...
  let k = 0;
  strList.forEach((entry) => {
    if (vowels.includes(entry)) k += 1;
  });
  return k;
}
...
function isPalindrome(str) {
  ...
  return (
    strR.replaceAll(regExp, '').toLowerCase() ===
    str.replaceAll(regExp, '').toLowerCase()
  );
}
...
function findLongestWord(sentence) {
  ...
  let k = '';
  strList.forEach((entry) => {
    if (entry.length > k.length) k = entry;
  });
  return k;
}
...
function reverseWords(str) {
  ...
  strArr.forEach((entry, i) => {
    strArr[i] = entry.split('').reverse().join('');
  });
  return strArr.join(' ');
}
...
function invertCase(str) {
  ...
  strArr.forEach((entry, i) => {
    if (entry === entry.toLowerCase()) {
      strArr[i] = entry.toUpperCase();
    } else {
      strArr[i] = entry.toLowerCase();
    }
  });
  return strArr.join('');
}
...
function extractNameFromTemplate(value) {
  ...
  return `${strArr[1]} ${strArr[2].substring(0, strArr[2].length - 1)}`;
}
...
function encodeToRot13(str) {
  ...
  const alphabetArrCoded =
    'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'.split('');
  let res = '';
  str.split('').forEach((entry) => {
    if (alphabetArr.indexOf(entry) !== -1) {
      res = res.concat(alphabetArrCoded[alphabetArr.indexOf(entry)]);
    } else {
